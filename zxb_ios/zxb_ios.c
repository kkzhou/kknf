...
struct zxb_ios* zxb_ios_get()
{
    ...
    ios->ctx_fd_vector = (struct zxb_contex*)malloc(sizeof (struct zxb_context) * ios->ctx_fd_vector_size);
    memset(ios->ctx_fd_vector, sizeof (struct zxb_context) * ios->ctx_fd_vector_size, 0);
    
    ios->ctx_id_vector_size = 1000;
    ios->ctx_id_vector = (uint32_t*)malloc(sizeof (int) * ios->ctx_id_vector_size)
    memset(ios->ctx_id_vector,sizeof (int) * ios->ctx_id_vector_size, 0);
...
int zxb_ios_tcp_listen_at(struct zxb_ios *ios, uint32_t net_ip, uint16_t net_port)
{
    ...
    struct zxb_context *new_ctx = zxb_context_create(listen_socket, 0, T_TCP);
