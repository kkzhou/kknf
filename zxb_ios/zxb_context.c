...
int zxb_context_create_tcp_listener(uint32_t net_ip, uint16_t net_port, int backlog)
{
  ...
  memset(&addr, sizeof (struct socketaddr_in), 0);
  
  addr.sin_family = AF_INET;
  addr.sin_addr = net_ip;
                        ...
                        uint32_t net_my_ip, uint16_t net_my_port)
{
  ...
  if (net_my_ip != 0) {
      ...
      memset(&myaddr, sizeof (struct socketaddr_in), 0);
  
      myaddr.sin_family = AF_INET;
      myaddr.sin_addr = net_to_ip;
