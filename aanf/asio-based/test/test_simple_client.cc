...
public:
    ...
    void PrepareDataThenSend() {
        ...
        ReqToBF req;
        req.type_ = TestPacketBase::PacketType.T_REQ_TO_BF;
        req.a_ = a++;
        req.b_ = b--;
        req.seq_ = seq++;
        int len = sizeof(ReqBF);
        ...
        if (!skinfo) {
            IPAddress addr = IPAddress.fromstring(server_ip_);
            TCPEndpoint remote_endpoint(addr, server_port_);
            ToConnectThenWrite(endpoint, send_buf);
        } else {
