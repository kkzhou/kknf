...
public:
    // Two interfaces,
    // the only two should be overrided in derived classes

    // ProcessData is the function which implimented by user to dealing with bisiness logic
    // return value:
    // <0:  error happened
    // 0:  this is a server socket, and to close(never happens in a Client)
    // 1:  this is a client socket, and to set idle
    ...
    // PrepareDataThenSend is to construct the request to send to server.
...
public:
    ...
    int UDPToWrite(UDPEndpoint to_endpoint, std::vector<char> &buf_to_send) {
        ...
        if (!udp_socket_->in_use()) {
            // not in sending state, so call async_send_to
